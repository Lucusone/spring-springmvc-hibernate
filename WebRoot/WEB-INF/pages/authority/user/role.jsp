	...
	<form id="roleForm" method="post">
		<div class="form-group">
			<select id="roleId" name="roleId" multiple="multiple" style="width: 100%">
				<c:forEach var="xtRole" items="${xtRoles}">
					<option value="${xtRole.roleId}" <c:forEach var="xtUserRole" items="${xtUserRoles}"> <c:if test="${xtRole.roleId==xtUserRole.roleId}">selected="selected"</c:if> </c:forEach>> ${xtRole.roleName} </option>
				</c:forEach>
			</select>
		</div>
...
function roleSubmit() {
    $.ajax({
        type: 'post',
        url: '${basePath}/authority/xtuser/role/' + roleUserId,
